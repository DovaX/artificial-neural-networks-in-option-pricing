...
def plot_particular_moneyness(pdata, cpflag):
    ...
    plot1.construct_plot(name, "Moneyness $S/X$", "$"+cpflag+"/X$", save="Moneyness_"+str(
        cpflag)+".png", xymin=[0.8, 0.0], xymax=[1.3, 0.25], scatter=True, vlines=vlines)
...
def plot_close(underlying):
    ...
    plot2.construct_plot("S&P500 daily closing price", "Date", "S", save="close.png",
                         xticks_bool=True, xymin=[0, 1000], xymax=[761, 1600], figsize=(10, 5))
...
def plot_returns(underlying):
    ...
    plot2.construct_plot("Returns", "Date", "Returns", save="returns.png", xticks_bool=True, xymin=[
                         0, -0.025], xymax=[761, 0.025], figsize=(10, 5))
...
def plot_volatilities(underlying):
    ...
    plot2 = MyPlot()
    plot2.append_data(
        underlying['day'], underlying['volatility5'], 'k', '$\sigma_{MA5}$', linewidth=1.0)
    plot2.append_data(
        underlying['day'], underlying['volatility20'], 'r', '$\sigma_{MA20}$', linewidth=1.5)
    plot2.append_data(
        underlying['day'], underlying['volatility60'], 'g', '$\sigma_{MA60}$', linewidth=2.0)
    plot2.append_data(underlying['day'], underlying['volatility100'],
                      'b', '$\sigma_{MA100}$', linewidth=2.5)
    plot2.construct_plot("Volatility", "Date", "Volatility ($\sigma$)", save="volatility.png",
                         xticks_bool=True, xymin=[0, 0], xymax=[761, 0.20], figsize=(10, 5))
...
def volatility_table(calls_mod):
    ...
    (order:  0.97, t1,t2,t3, 1.00 t1,t2,t3, 1.05 t1,t2,t3)"""
    calls_mod_implied_vol = [calls_mod[i]['implied'].mean()
                             for i in range(0, 9)]
    print("\\% & ".join([str(round(item*100, 2))
                         for item in calls_mod_implied_vol]))
    calls_mod_vol5 = [calls_mod[i]['volatility5'].mean() for i in range(0, 9)]
    print("\\% & ".join([str(round(item*100, 2)) for item in calls_mod_vol5]))
    calls_mod_vol20 = [calls_mod[i]['volatility20'].mean()
                       for i in range(0, 9)]
    ...
    calls_mod_vol100 = [calls_mod[i]['volatility100'].mean()
                        for i in range(0, 9)]
    print("\\% & ".join([str(round(item*100, 2))
                         for item in calls_mod_vol100]))
    calls_mod_volgarch = [calls_mod[i]['vol_garch'].mean()
                          for i in range(0, 9)]
    print("\\% & ".join([str(round(item*100, 2))
                         for item in calls_mod_volgarch]))
    ...
    calls_mod_vol20_means = [calls_mod[0+i]['volatility20'].append(calls_mod[1+i]['volatility20'], ignore_index=True).append(
        calls_mod[2+i]['volatility20'], ignore_index=True).mean() for i in range(0, 9, 3)]
    ...
    calls_mod_vol100_means = [calls_mod[0+i]['volatility100'].append(calls_mod[1+i]['volatility100'], ignore_index=True).append(
        calls_mod[2+i]['volatility100'], ignore_index=True).mean() for i in range(0, 9, 3)]
    ...
    calls_mod_implied_vol_means = [calls_mod[0+i]['implied'].append(calls_mod[1+i]['implied'], ignore_index=True).append(
        calls_mod[2+i]['implied'], ignore_index=True).mean() for i in range(0, 9, 3)]
